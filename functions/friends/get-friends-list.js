      ...
      .then((friendships) => {
        const userFriendships =
          friendships.filter((friendship) =>
            (friendship.uidTo === userUid || friendship.uidFrom === userUid) && friendship.status === 'ACCEPTED'
          );

        Promise.all(
          userFriendships
            .map((friendship) => {
              const friendId = friendship.uidTo === userUid ? friendship.uidFrom : friendship.uidTo;
